...
//int main()
//{
//    // Create a synthetic radial distortion map for testing
//    const int w = 512, h = 512;
//    Mat distortionMap = Mat::zeros(h, w, CV_32FC1);
//
//    // Synthetic: center=1.0, corners=0.0
//    Point center(w / 2, h / 2);
//    float maxR = sqrtf((w / 2.0f) * (w / 2.0f) + (h / 2.0f) * (h / 2.0f));
//    for (int y = 0; y < h; ++y)
//    {
//        float* rowPtr = distortionMap.ptr<float>(y);
//        for (int x = 0; x < w; ++x)
//        {
//            float dx = x - center.x;
//            float dy = y - center.y;
//            float dist = sqrtf(dx * dx + dy * dy);
//            float val = 1.0f - (dist / maxR);
//            rowPtr[x] = (val < 0.f) ? 0.f : val;
//        }
//    }
//
//    // Desired grid size
//    int   Grid_x = 20;
//    int   Grid_y = 20;
//    float LowThreshold = 0.3f; // High-distortion threshold
//
//    vector<Point> adaptivePoints;
//    GenerateAdaptiveGrid_HAG_WeightedVoronoi(
//        distortionMap,
//        adaptivePoints,
//        Grid_x,
//        Grid_y,
...
//    );
//
//    cout << "Final number of points: " << adaptivePoints.size() << endl;
//    return 0;
//}
